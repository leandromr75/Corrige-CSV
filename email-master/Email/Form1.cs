...
namespace Email
{
    public partial class Form1 : Form
    {
        ...
        public string DetectFileEncoding(String path)
        {
        ...
        private void abrirToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            Encoding encoding = Encoding.GetEncoding(this.DetectFileEncoding(path));
            
            //abre o arquivo csv
            string[] Linha = System.IO.File.ReadAllLines(path, encoding);
            ...
            for (Int32 i = 0; i < Linha.Length; i++)
            {
                ...
                //toda linha válida começa com "AI-"
                if (campos[0].Contains("AI-") && i > 0)
                {
                    tempCampos = i;
                    vai = true;
                }

                //verifica se a linha é irregular e contém dados da linha anterior
                if (!campos[0].Contains("AI-") && i > 0)
                {
                    for (int j = 0; j < campos.Length; j++)
                    {
                        DateTime data;
                        bool dataValida = DateTime.TryParse(campos[j], out data);

                        //identifica a célula que contém a data e ajusta as informações fora de ordem
                        if (dataValida && vai)
                        {
                            string[] camposTemp = Linha[tempCampos].Split(Convert.ToChar(";"));
                            if (camposTemp.Length >= 14 && campos.Length >= 14)
                            {
                                camposTemp[11] += tempOBS;          //insere na observação os itens espalhados 
                                camposTemp[12] = campos[j];         //ajusta a data na linha correta
                                camposTemp[13] = campos[j + 1];     //ajusta o nome na linha correta

                                if (!string.IsNullOrEmpty(camposTemp[12]) && !string.IsNullOrEmpty(camposTemp[13]))  
                                {
                                    dt.Rows.Add(camposTemp);
                                }
                                vai = false;
                            }
                        }
                        if (!dataValida)
                        {
                           tempOBS += " " + campos[j];        //recolhe os fragmentos da observação
                        }
                    }
                }
                else
                {
                    if (!string.IsNullOrEmpty(campos[12]) && !string.IsNullOrEmpty(campos[13]))
                    {
                        dt.Rows.Add(campos);                 //insere a linha onde não houve necessidade de ajustar as informações
                        tempOBS = "";
                    }
                }
